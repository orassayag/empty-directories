...
class InitiateService {
  ...
  validateArrays() {
    [
      ...
      if (!validationUtils.isValidArray(value)) {
        throw new Error(
          `Invalid or no ${key} parameter was found: Expected a array but received: ${value} (1000007)`
        );
      }
    });
  }
  ...
  validateSpecial() {
    ...
    if (fileUtils.isFilePath(SCAN_PATH)) {
      throw new Error(
        `The path SCAN_PATH parameter needs to be a directory path but it's a file path: ${SCAN_PATH} (1000008)`
      );
    }
  }
  ...
  validateDirectories() {
    ...
    [
      ...
      if (!fileUtils.isDirectoryPath(value)) {
        throw new Error(
          `The parameter path ${key} marked as directory but it's a path of a file: ${value} (1000009)`
        );
      }
    });
  }
